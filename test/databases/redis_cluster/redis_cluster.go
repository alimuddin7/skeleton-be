...
func InitializeRedisCluster(conn *redis.ClusterClient, log zerolog.Logger) RedisCluster {
	return &redisCluster{
		Client: conn,
		Logs:   log,
	}
...
func (r *redisCluster) GetQueue(ctx context.Context, d time.Duration, key string) ([]string, error) {
	return r.Client.BRPop(ctx, d, key).Result()
}

func (r *redisCluster) PushJobRedis(ctx context.Context, key string, value interface{}) error {
	return r.Client.RPush(ctx, key, value).Err()
}

func (r *redisCluster) SaveRedisExp(ctx context.Context, exp time.Duration, key string, val interface{}) error {
	return r.Client.Set(ctx, key, val, exp).Err()
}

func (r *redisCluster) GetRedisKey(ctx context.Context, key string) (string, error) {
	return r.Client.Get(ctx, key).Result()
}

func (r *redisCluster) Increment(ctx context.Context, key string) error {
	return r.Client.Incr(ctx, key).Err()
}

func (r *redisCluster) HealthCheck(ctx context.Context) hModels.DataHealthCheck {
